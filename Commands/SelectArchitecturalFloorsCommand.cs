...
namespace RevitAddIn
{
    ...
    class SelectArchitecturalFloorsCommand : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            UIDocument uiDocument = commandData.Application.ActiveUIDocument;
            Document document = uiDocument.Document;
            SelectArchitecturalFloorsForm architecturalFloorsForm = new SelectArchitecturalFloorsForm(commandData);

            try
            {
                using (Transaction trans = new Transaction(document, "Select Architectural Floors"))
                {
                    trans.Start();
                    ...
                    var architecturalFloorTypes = new List<FloorType>();
                    foreach (var floorType in GetElements.GetArchitecturalFloorTypes(document))
                    {
                        if (!floorType.Name.Contains("Generic"))
                        {
                            architecturalFloorTypes.Add(floorType);
                        }
                    }
                    SetParametersFromDatabase(excelData, new List<string>() { "Unit Price", "Embodied Energy", "Social Score" }, architecturalFloorTypes);


                    // open form
                    architecturalFloorsForm.ShowDialog();
