...
namespace RevitAddIn
{
    ...
    class SelectInteriorWallsCommand : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            UIDocument uiDocument = commandData.Application.ActiveUIDocument;
            Document document = uiDocument.Document;
            SelectInteriorWallsForm interiorWallsForm = new SelectInteriorWallsForm(commandData);
            ...
            try
            {
                using (Transaction trans = new Transaction(document, "Select Internal Walls"))
                {
                    ...
                    var interiorWallTypes = new List<WallType>();
                    foreach (var wallType in GetElements.GetInteriorWallTypes(document))
                    {
                        if (!wallType.Name.Contains("Generic"))
                        {
                            interiorWallTypes.Add(wallType);
                        }
                    }
                    SetParametersFromDatabase(excelData, new List<string>() { "Unit Price", "Embodied Energy", "Social Score"}, interiorWallTypes);
                    

                    // open form
                    interiorWallsForm.ShowDialog();
